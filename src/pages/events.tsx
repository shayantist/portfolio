...
import { EventCard, Page, SEOHead } from "../components";
...
export default function EventsPage({ pageTitle, blocks, events }: any) {
  return (
    <Page>
...
export async function getServerSideProps() {
  const notion_page_url = "acd49d01a5b44f2c90b602aa771a6c93";
  ...
  const eventsDBBlock = blocks.pop();
  const eventsDB: any =
    eventsDBBlock &&
    (await getDatabase("a9ea839a3029489f848a2c10e7340c35", "Date"));
  ...
  for (const event of eventsDB) {
    ...
    let imageURL;
    if (event.properties.Images.files.length > 0) {
      let image = event.properties.Images.files[0];
    ...
    const blurb = textToMarkdown(event.properties.Blurb.rich_text);
